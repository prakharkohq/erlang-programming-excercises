...
start(Name, HandlerList) ->
  register(Name, spawn(fun() -> init(HandlerList) end)), ok.
...
handle_msg({swap_handlers, OldHandler, NewHandler}, LoopData) ->
  case lists:keysearch(OldHandler, 1, LoopData) of
    ...
    {value, {OldHandler, Data}} ->
      {_, OldData} = OldHandler:terminate(Data),
      NewData = NewHandler:init(OldData),
      NewLoopData = lists:keyreplace(OldHandler, 1, LoopData, {NewHandler, NewData}),
      {ok, NewLoopData}
  end;
